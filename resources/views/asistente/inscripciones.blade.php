    ...
    <div class="py-12 bg-white">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-300 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-6 text-gray-900 dark:text-gray-100">
                    @foreach ($inscripciones as $inscripcion)
                    <div class="flex flex-col justify-between rounded-lg overflow-hidden shadow-lg bg-white dark:bg-gray-700 transition hover:shadow-xl">
                        ...
                        <!-- Event Content -->
                        <div class="px-6 py-4">
                            <h3 class="font-bold text-lg mb-2 text-gray-800 dark:text-white">{{ $inscripcion->evento->nombre }}</h3>
                            <p class="text-gray-700 dark:text-gray-300 text-sm mb-4">{{ $inscripcion->evento->descripcion }}</p>

                            <!-- Event Status -->
                            @if($inscripcion->evento->estado == 'terminado')
                                <div class="flex items-center bg-blue-500 text-white text-sm font-bold rounded-lg px-4 py-2 mb-4">
                            ...
                            @else
                                <p class="text-white text-sm mb-4">Fecha: {{ $inscripcion->evento->fecha }}</p>
                            @endif
                        </div>

                        <!-- Tags and Button -->
                        <div class="px-6 pb-4 flex flex-col items-center">
                            <span class="inline-block bg-gray-200 dark:bg-gray-600 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 dark:text-gray-300 mb-2">#{{ $inscripcion->evento->categoria->nombre }}</span>
                            ...
                            <x-modal>
                                ...
                                <div class="w-full p-4 flex flex-col items-center">
                                    ...
                                    <span class="bg-gray-200 dark:bg-gray-600 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 dark:text-gray-300">Estado: {{ $inscripcion->evento->estado }}</span>
                                </div>
                            </x-modal>
